...
// Substraction of decimals with aligned scales
int s21_sub_aligned(s21_decimal value_1, s21_decimal value_2,
                    s21_decimal *result);
...
int s21_sub(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (!sign1 && !sign2) {  // Both numbers positive
    ...
    if (s21_is_greater(value_1, value_2)) {
      ctl = s21_sub_aligned(value_1, value_2, result);
    } else if (s21_is_greater(value_2, value_1)) {
      ctl = s21_sub_aligned(value_2, value_1, result);
    } else
      ctl = -1;
    if (ctl >= 0) {
